 ...
 * - Violates MISRA 2004 Required Rule 18.4, declaration of union type or
...
#define __CM4_CMSIS_VERSION (( \
    __CM4_CMSIS_VERSION_MAIN << 16) | __CM4_CMSIS_VERSION_SUB \
)
 ...
 */
            ...
            #warning "Device is without an FPU (check __FPU_PRESENT)"
            #define __FPU_USED 0
        #endif
    #else
        #define __FPU_USED 0
...
/* check device defines and use defaults */
        ...
        #warning "__CM4_REV not defined in device header file"
        #warning "Switch to default!"
    #endif
    #ifndef __FPU_PRESENT
        #define __FPU_PRESENT 0
        #warning "__FPU_PRESENT not defined in device header file"
        #warning "Switch to default!"
    #endif
...
typedef union {
    struct {
        uint32_t ISR:9; /* 0..8 Exception number */
...
#define FPU_MVFR0_FP_rounding_modes_Msk ( \
    0xFUL << FPU_MVFR0_FP_rounding_modes_Pos \
)
...
static __INLINE void NVIC_EnableIRQ(IRQn_Type IRQn) {
    ...
    NVIC->ISER[(uint32_t)((int32_t)IRQn) >> 5] = (uint32_t) (
        1 << ((uint32_t)((int32_t)IRQn) & (uint32_t)0x1F)
    ); /* enable interrupt */
}
...
/**
 * Encode Priority
 * This function encodes the priority for an interrupt with the given
 * priority group, preemptive priority value and sub priority value.
 * In case of a conflict between priority grouping and available
 * priority bits (__NVIC_PRIO_BITS) smallest possible priority group is set.
 ...
 * In case of a conflict between priority grouping and available
 * priority bits (__NVIC_PRIO_BITS) the samllest possible priority
 * group is set.
...
static __INLINE void NVIC_SystemReset(void) {
    /**
     * Ensure all outstanding memory accesses included buffered
     * write are completed before reset.
     */
    __DSB();
...
/**
 * ITM Check Character
 * This function checks external variable ITM_RxBuffer whether
 * a character is available or not. It returns '1' if a character is
 * available and '0' if no character is available.
